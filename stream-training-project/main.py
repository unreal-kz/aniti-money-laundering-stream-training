...
def main():
    ...
    if not os.listdir(train_json_dir) or not os.listdir(test_json_dir):
        print("Splitting JSON files into training and testing sets...")
        split_json_files(jsons_dir, train_json_dir, test_json_dir, test_size=0.2)
    else:
        print("Train-test split already exists. Skipping this step.")

    # Step 2: Generate graphs and extract features from train JSONs
    print("Generating graphs and extracting features from training JSON files...")
